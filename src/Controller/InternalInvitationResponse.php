...
final class InternalInvitationResponse
{
    ...
    public function __invoke(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
    {
        ...
        try {
            ...
            if (!str_starts_with($principal, $tenantPrefix) || (!isset(self::STATUSES[$status]) && $status !== 'cancelled')) {
                throw new \RuntimeException('Invalid response context');
            }
            if ($mailAccountId < 1 || filter_var($email, FILTER_VALIDATE_EMAIL) === false || strlen($raw) > 2 * 1024 * 1024) {
            ...
            if ($status === 'cancelled') {
                ...
                return $this->json($response, ['stored' => false, 'cancelled' => true, 'status' => $status]);
            }
            ...
            $message = $this->container->get(ImipMessageFactory::class)->reply($calendar->serialize(), $email);
            $this->container->get(CalendarBridgeClient::class)->sendImipMessage($message);

            return $this->json($response, ['stored' => true, 'status' => $status]);
